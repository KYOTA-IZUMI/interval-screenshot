...
async function createDailyReport() {
  try {
    ...
    <head>
      <meta charset="UTF-8">
      <title>IntervalScreenshot Daily Report - ${today}</title>
      <style>
        body {
    ...
    <body>
      <h1>IntervalScreenshot Daily Report - ${today}</h1>
    ...
    if (appConfig.notifications) {
      new Notification({
        title: 'IntervalScreenshot - Daily Report',
...
async function initGitRepository() {
  ...
  if (!isRepo) {
    await git.init();
    await git.addConfig('user.name', 'IntervalScreenshot App');
    await git.addConfig('user.email', 'intervalscreenshot@example.com');
  }
}
...
function updateMenu() {
  try {
    ...
    tray.setToolTip(`IntervalScreenshot - ${isRecording ? 'Recording' : 'Stopped'}`);
  } catch (error) {
